...
Health <- top_schools_percentage |>
  ...
  geom_text(aes(y = 2), vjust = 0, size = 8, color = "white") +  # Use Cambria font for text
  coord_flip()+
  theme_minimal()+
  labs(x = "", y = "", 
  ...
  theme(axis.text.x = element_blank(),  # Remove y-axis tick labels
        ...
        plot.subtitle =  element_text(hjust = 0, size = 28))



ggsave("Health.jpg", plot = Health, width = 12, height = 8, units = "in", dpi = 200)
...
ggsave("engneer.jpg", plot =  Engineering, width = 12, height = 8, units = "in", dpi = 200)


bussiness <- top_schools_percentage |>
  filter(Theme == "Business") |> 
  mutate(School = fct_reorder(School,Total_Scholarships)) |> 
  ggplot(aes(School,Total_Scholarships, label = paste0(Percentage,"%")))+
  geom_col(fill = "#9e0059", color = "white")+
  geom_text(aes(y = 2), vjust = 0, size = 8, color = "white") +  # Use Cambria font for text
  coord_flip()+
  theme_minimal()+
  labs(x = "", y = "", 
       title = "Leading Business  Scholarship Recipients -  Makerere Univerity | 2023",
       subtitle = "These top schools take 22% of the Total scholarships awarded in Business  assessed (409)" )+
  theme(axis.text.x = element_blank(),  # Remove y-axis tick labels
        axis.ticks.x = element_blank(),
        axis.text.y = element_text(size = 24), 
        plot.title = element_text(hjust = 0, size = 32, face = "bold"),
...
ggsave("business.jpg", plot = bussiness, 
       width = 12, height = 8, units = "in", dpi = 200)
